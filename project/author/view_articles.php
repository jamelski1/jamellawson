...
if(!isset($_POST['submit']))	// we need to present the form and get the data
  {
    ...
    $query = "SELECT title, abstract, theTime, status, rating, articles.article_id
    ...
    if ($result) {
		...
		if ($numberofrows > 0){
			...
			$contentStr .= "
				<center>You haven't submitted any articles!</center>
				";
			}
        ...
        for($i=0; $i < $numberofrows; $i++) {
			...
			$contentStr .= "\n <td>" . $row['status'] . "</td>";
			$contentStr .= "\n <td>" . $row['rating'] . "</td>";
			$contentStr .= "\n <td> <a href='submitted_articles.php?fid=" . $row['article_id'] . "'>View Article</a></td>";
            $contentStr .= "\n </tr>";
        }

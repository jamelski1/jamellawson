...
#this takes two dictionaries in as an argument
#for example Combine(TableRead("wiki.txt"), TableRead("authors.txt"))
...
#note to use the above definition you have to call it like this:
#Combine(TableRead("wiki.txt"),TableRead("authors.txt"))
                
def MatchB(table):
    #empty list initialized to store the matching titles
    matchingTitles = []
    #iterates through all of the keys in the dictionary
    for i in range(len(table)):
        #checks if the table's first letter starts with a B
        if table[i][1][0] == 'B':
            #adds the matching B titles to the matching titles
            matchingTitles += [table[i][1]]
            
    #returns the matching titles from the table that start with B
...
def Write(myFile = Combine(TableRead("wiki.txt"),TableRead("authors.txt"))):
    f = open('combined.txt', 'w')
    for variable in myFile:
        f.write(str(myFile[variable]) + "\n")  
    f.close()
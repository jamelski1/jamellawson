...
if(!isset($_POST['submit']))	// we need to present the form and get the data
  {
	...
	if($pageStr = file_get_contents($TEMPLATES_DIR . 'default.html'))
	{
			...
			<form action="insertebm.php" method="post">
    ...
    $db->close();
      ...
      $contentStr .= '<form action="insertreviewer.php" method="post">
    ...
    $db->close();
		...
		$contentStr .= '<input type="submit" value="Add Reviewer"/>';
    ...
    //Tables with roles and their associated members
	$contentStr .= "<p>
    <br/>
    <br/>
  ...
  $query = "SELECT DISTINCT firstname, lastname, email
	FROM people, role_user, role
	WHERE people.people_id = role_user.people_id AND role_user.role_id = 4
	ORDER BY lastname;";
